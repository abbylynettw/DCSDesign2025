...
namespace WinformUI.CADHelper
{
    ...
    public static partial class BlockTools
    {
        ...
        public static BlockReference CreatDwgFileBlock(this Document doc,string blockPath, string blockName)
        {
            BlockReference br = null;            
            //执行代码            
            doc.Database.ImportBlocksFromDwg(blockPath);
            using (Transaction tran = doc.Database.TransactionManager.StartTransaction())
            {
                br = doc.Database.CurrentSpaceId.CreateBlockReference(blockName, Point3d.Origin);               
            }           
            return br;
        }
        public static BlockReference CreatDwgFileBlock(this Document doc, string blockName)
        {
            BlockReference br = null;
            using (Transaction tran = doc.Database.TransactionManager.StartTransaction())
            {
        ...
        /// <param name="scale">缩放比例</param>
        /// <returns>返回块参照的Id</returns>
        public static BlockReference CreateBlockReference(this ObjectId spaceId, string blockName, Point3d position)
        {
            Database db = spaceId.Database;//获取数据库对象
            //以读的方式打开块表
            BlockTable bt = (BlockTable)db.BlockTableId.GetObject(OpenMode.ForRead);
            ...
            //以写的方式打开空间（模型空间或图纸空间）
            var blockTableRecord = (BlockTableRecord)spaceId.GetObject(OpenMode.ForWrite);
            //创建一个块参照并设置插入点
            BlockReference br = new BlockReference(position, bt[blockName]);
            return br;
        }
        public static bool HasBlockReference(this Database db, string blockName)
        {
            //以读的方式打开块表
            BlockTable bt = (BlockTable)db.BlockTableId.GetObject(OpenMode.ForRead);
            //如果没有blockName表示的块，则程序返回
            ...
            if (record.HasAttributeDefinitions)
            {
                ...
                foreach (ObjectId id in record)
                {
                    ...
                    if (attDef != null)
                    {
                        ...
                        {
                            //设置属性值
                            attribute.TextString = attNameValues[attDef.Tag.ToUpper()];
                        }
                        //向块参照添加属性对象
        ...
        public static void UpdateAttributesInBlock(this ObjectId blockRefId, Dictionary<string, string> attNameValues)
        {
            ...
            if (blockRef != null)
            {
                ...
                foreach (ObjectId id in blockRef.AttributeCollection)
                {
                    ...
                    //判断是否包含指定的属性名称
                    if (attref is { } && attNameValues.ContainsKey(attref.Tag.ToUpper()))
                    {
        ...
        /// <returns>返回指定名称的块属性值</returns>
        public static string GetAttributeInBlockReference(DrawingUtility util, BlockReference bref, string attributeName)
        {
            string attributeValue = string.Empty; // 属性值
            // 遍历块参照的属性
            foreach (ObjectId attId in bref.AttributeCollection)
            {
                // 获取块参照属性对象
                AttributeReference attRef = (AttributeReference)util.GetDBObject(attId, OpenMode.ForRead);
                //判断属性名是否为指定的属性名
                if (attRef.Tag.ToUpper() == attributeName.ToUpper())
                {
                    attributeValue = attRef.TextString;//获取属性值
        ...
        /// <param name="util"></param>
        /// <returns>返回指定名称的块属性值</returns>
        public static bool IsHasTag(DrawingUtility util, BlockReference bref, string attributeName)
        {
            // 遍历块参照的属性
            foreach (ObjectId attId in bref.AttributeCollection)
            {
        ...
        public static bool SetTagValue(DrawingUtility util,BlockReference bref,string attributeName,string value)
        {
            // 遍历块参照的属性
            foreach (ObjectId attId in bref.AttributeCollection)
            {
        ...
        public static List<BlockReference> GetAllDynBlockReferences(this Database db, string blockName)
        {
            ...
            BlockTable bt = (BlockTable)trans.GetObject(db.BlockTableId, OpenMode.ForRead);
            var blockTableRecord = (BlockTableRecord)trans.GetObject(bt[blockName], OpenMode.ForRead);
            var blocks = (from b in db.GetEntsInDatabase<BlockReference>()
        ...
        /// <param name="isErased"></param>
        /// <returns></returns>
        public static Dictionary<string, string> GetBlocksDicAndDelBlock(this Database db, DrawingUtility util, string[] tagNames, bool isErased = false)
        {
            Dictionary<string, string> dic = new Dictionary<string, string>();
            List<BlockReference> brs = util.GetAllEntityInCurrentSpace<BlockReference>();
            foreach (var br in brs)
            {
                var tags = util.GetAttributesTagInBlockReference(br);
                if (tags.Count == 0) continue;                
                var intersect = tagNames.ToList().Intersect(tags).ToList();
                if (intersect.Count>0)
                {
                    util.GetAttributesInBlockReference(br, dic);
                    if (isErased)
                    {
                        br.UpgradeOpen();
                        br.Erase();
        ...
        /// <returns></returns>
        public static string GetBlockTagValue(this Database db, DrawingUtility util, string tagName)
        {
            List<BlockReference> brs = util.GetAllEntityInCurrentSpace<BlockReference>();
            foreach (var br in brs)
            {
                if (IsHasTag(util, br, tagName))
                {
                    var tagValue = GetAttributeInBlockReference(util, br, tagName);
                    return tagValue;
                }
            }
            return "";
        }
        public static string GetBlockTagValueInDb(this Database db, DrawingUtility util, string tagName)
        {
            List<BlockReference> brs = util.db.GetEntsInDatabase<BlockReference>();
            foreach (var br in brs)
            {
                if (IsHasTag(util, br, tagName))
                {
        ...
        public static string GetBlockTagValue(this Database db, DrawingUtility util, string tagName,string layerName)
        {
            List<BlockReference> brs = util.GetAllEntityInCurrentSpace<BlockReference>();
            foreach (var br in brs)
            {
                if (br.Layer != layerName) continue;
                if (IsHasTag(util, br, tagName))
                {
                    var tagValue = GetAttributeInBlockReference(util, br, tagName);
                    return tagValue;
        ...
        public static void SetBlockTagValue(this Database db, DrawingUtility util, string tagName, string value)
        {
            List<BlockReference> brs = util.GetAllEntityInCurrentSpace<BlockReference>();
            foreach (var br in brs)
            {
                var boo = SetTagValue(util, br, tagName, value);
                if (boo) return;
            }
        }
        public static void SetBlockTagValueInDB(this Database db, DrawingUtility util, string tagName, string value)
        {
            List<BlockReference> brs = util.db.GetEntsInDatabase<BlockReference>();
            foreach (var br in brs)
            {
        ...
        public static void SetBlockTagValue(this Database db, DrawingUtility util, string tagName, string value,string layerName)
        {
            List<BlockReference> brs = util.GetAllEntityInCurrentSpace<BlockReference>();
            foreach (var br in brs)
            {
        ...
        public static BlockReference GetOneBlock(this Database db,string blockName)
        {
            List<BlockReference> brs = db.GetEntsInDatabase<BlockReference>();
            var brCncs = brs.Where(b => (b != null && b.ObjectId != ObjectId.Null) && (b.Name == blockName)).ToList();
            if (brCncs.Count == 0) return null;
            if (brCncs.Count == 1) return brCncs[0];  
            
            double len = brCncs[0].Position.GetVectorTo(Point3d.Origin).Length;
            int index = 0;
            for (int i = 0; i < brCncs.Count; i++)
            {
                var vecLength = brCncs[i].Position.GetVectorTo(Point3d.Origin).Length;
                if (vecLength < len)
                {
                    len = vecLength;
                    index = i;
        ...
        public static List<BlockReference> GeBlocks(this Database db, string blockName)
        {
            List<BlockReference> brs = db.GetEntsInModelSpace<BlockReference>();
            var str = "";
            foreach (var br in brs)
            {
                str+=br.Name;
            }
            Autodesk.AutoCAD.ApplicationServices.Core.Application.ShowAlertDialog(str);
            var blocks = brs.Where(b => (b != null && b.ObjectId != ObjectId.Null) && (b.Name == blockName)).ToList();
            return blocks;
        }
        public static List<BlockReference> GetDynamicBlocks(this Database db, string blockName)
        {
            List<BlockReference> brs = db.GetEntsInModelSpace<BlockReference>();          
            var blocks = brs.Where(b => (b != null && b.ObjectId != ObjectId.Null) && (b?.GetBlockName() == blockName))?.ToList();
            return blocks;
        }
        /// <summary>
        /// 导入外部文件中的块
        ...
        /// <param name="sourceFileName">包含完整路径的外部文件名</param>
        public static void ImportBlocksFromDwg(this Database destDb,Database sourceDb,string sourceFileName)
        {
                ...
                if (prop.ReadOnly == false && prop.PropertyName == propName)
                {
                    ...
                    switch (prop.PropertyTypeCode)
                    {
                        ...
                        case (short)DynBlockPropTypeCode.Real://实型
                            ...
                            break;
                        case (short)DynBlockPropTypeCode.String://实型  
                            if (propName == "可见性1")
                            {
                                prop.Value = value.ToString();
                            }
                            else if (propName == "角度1")
                            {
                                double angle = 0;
                                double.TryParse(value.ToString(), out angle);
                                prop.Value = angle;
                            }
                            else
                            {
