...
namespace WinformUI.CADHelper
{
    ...
    public static class TextStyleTools
    {
        ...
        public static ObjectId AddTextStyle(this Database db, string styleName, string fontFilename, string bigFontFilename,double width,double height)
        {
            //打开文字样式表
        ...
        /// <param name="fontFilename">字体文件名</param>
        /// <returns>返回添加的文字样式的Id</returns>
        public static ObjectId AddTextStyle(this Database db, string styleName, string fontFilename,double width,double height)
        {
            return db.AddTextStyle(styleName, fontFilename, "",width,height);
        }
        ...
        public static bool SetCurrentTextStyle(this Database db, string styleName)
        {
            ...
            if (!st.Has(styleName))
            {
                db.AddTextStyle("CNCS", "txt.shx", 0.7, 2.5);
            }
        ...
        /// <param name="styleName">文字样式名</param>
        /// <returns>如果删除成功返回true，否则返回false</returns>
        public static ObjectId GetTextStyle(this Database db, string styleName)
        {
            var trans = db.TransactionManager;
            //打开文字样式表
            TextStyleTable st = (TextStyleTable)trans.GetObject(db.TextStyleTableId, OpenMode.ForRead);
            //如果不存在名为styleName的文字样式，则返回
            if (!st.Has(styleName)) return ObjectId.Null;
            //获取名为styleName的文字样式表记录的Id
            ObjectId styleId = st[styleName];
            return styleId;
        }
