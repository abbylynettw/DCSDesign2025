...
namespace WinformUI.CADHelper
{
    ...
    public static partial class LinqToCAD
    {
        ...
        /// <param name="isIgnoredLayout"></param>
        /// <returns>返回数据库中类型为T的实体</returns>
        public static List<T> GetEntsInDatabase<T>(this Database db,bool isIgnoredLayout=false) where T : Entity
        {
            return GetEntsInDatabase<T>(db, OpenMode.ForRead, false, isIgnoredLayout);
        }
        ...
        /// <returns>返回数据库中类型为T的实体</returns>
        public static List<T> GetEntsInDatabase<T>(this Database db, OpenMode mode, bool openErased,bool isIgnoredLayout) where T : Entity
        {
            ...
            if (entSelected.Status == PromptStatus.OK)
            {
                ...
                {                  
                    ...
                    if (t is BlockReference)
                    {
                        BlockReference br = t as BlockReference;
                        if (br.BlockTableRecord == ObjectId.Null)
                        {
                            continue;
                        }
                        BlockTableRecord btr = (BlockTableRecord)br.BlockTableRecord.GetObject(OpenMode.ForRead, false);
        ...
        /// <param name="openErased">是否打开已删除的实体</param>
        /// <returns>返回模型空间中类型为T的实体</returns>
        public static List<T> GetEntsInSpace<T>(this Database db, OpenMode mode,string layoutName) where T : Entity
        {
            Editor ed = Application.DocumentManager.MdiActiveDocument.Editor;
            //声明一个List类的变量，用于返回类型为T为的实体列表
            List<T> ents = new List<T>();
            ...
            if (entSelected.Status == PromptStatus.OK)
            {
                ...
                foreach (var id in entSelected.Value.GetObjectIds())
                {
                    //将实体强制转化为T类型的对象
                    //不能将实体直接转化成泛型T，必须首先转换成object类
                    T t = (T)(object)id.GetObject(mode);
                    ents.Add(t);//将实体添加到返回列表中
                }
            }
            return ents;//返回类型为T为的实体列表
        ...
        /// <param name="pt2">窗口的另一个角点</param>
        /// <returns>返回选择的实体</returns>
        public static List<Entity> SelectWindow(this Database db, Point3d pt1, Point3d pt2)
        {
            return SelectWindow(db, pt1, pt2, OpenMode.ForRead, false);
        }
        /// <summary>
        /// 选择窗口中及和窗口四条边界相交的实体
        /// </summary>
        ...
        /// <param name="pt2">窗口的另一个角点</param>
        /// <returns>返回选择的实体</returns>
        public static List<T> SelectWindow<T>(this Database db, Point3d pt1, Point3d pt2) where T:Entity
        {
            return SelectWindow<T>(db, pt1, pt2, OpenMode.ForRead, false);
        }
        /// <summary>
        /// 选择窗口中及和窗口四条边界相交的实体
        /// </summary>
        ...
        /// <param name="openErased">是否打开已删除的实体</param>
        /// <returns>返回选择的实体</returns>
        public static List<Entity> SelectWindow(this Database db, Point3d pt1, Point3d pt2, OpenMode mode, bool openErased)
        {
            var trans = db.TransactionManager;
            List<Entity> ents = new List<Entity>();
            Editor ed = Application.DocumentManager.MdiActiveDocument.Editor;
            var entSelected = ed.SelectWindow(pt1, pt2);
            if (entSelected.Status == PromptStatus.OK)
            {
                foreach (var id in entSelected.Value.GetObjectIds())
                {
        ...
        /// <param name="openErased">是否打开已删除的实体</param>
        /// <returns>返回选择的实体</returns>
        public static List<T> SelectWindow<T>(this Database db, Point3d pt1, Point3d pt2, OpenMode mode, bool openErased) where T:Entity
        {
            var trans = db.TransactionManager;
            List<T> ents = new List<T>();
            Editor ed = Application.DocumentManager.MdiActiveDocument.Editor;
            ...
            if (entSelected.Status == PromptStatus.OK)
            {
                foreach (var id in entSelected.Value.GetObjectIds())
                {
                    T ent = (Entity)(object)trans.GetObject(id, mode, openErased) as T;
                    if (ent != null)
                    {
                        ents.Add(ent);
                    }
