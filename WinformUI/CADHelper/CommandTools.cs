...
namespace WinformUI.CADHelper
{
    ...
    public static class CommandTools
    {
        public static void ExitCommand()
        {
            ...
            doc.SendStringToExecute(Convert.ToChar(27).ToString() + "\n", true, false, false);
        }
        public static void SendCommandSyn(this Document doc,string cmd)
        {
            ExitCommand();
            doc.SendStringToExecute(cmd + "\n", true, false, false);
        }
        public static void SendCommandSynUnExit(this Document doc, string cmd)
        {
            //ExitCommand();
            doc.SendStringToExecute(cmd + "\n", true, false, false);
        }
        ...
        //[DllImport("acad.exe", EntryPoint = "acedCmd", CharSet = CharSet.Ansi, CallingConvention = CallingConvention.Cdecl)]
        ...
        public static int AcedCmdCommand(this Editor ed, ResultBuffer args)
        {
            //由于acedCmd只能在程序环境下运行，因此需调用此语句
            if (!Application.DocumentManager.IsApplicationContext)
                return acedCommand(args.UnmanagedObject);
            else return 0;
        }
    }
}
