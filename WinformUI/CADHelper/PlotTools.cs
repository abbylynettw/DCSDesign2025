...
namespace WinformUI.CADHelper
{
    ...
    public static class PlotTools
    {
        ...
        public static PreviewEndPlotStatus PlotNew(this PlotEngine engine, Layout layout, PlotSettings ps, string fileName, bool showProgressDialog, bool plotToFile)
        {
            PlotProgressDialog plotDlg = null;
            if (showProgressDialog)//如果需要显示打印进度框，则创建
            {
                plotDlg = new PlotProgressDialog(false, 1, true);
                //获取去除扩展名后的文件名（不含路径）
                string plotFileName = SymbolUtilityServices.GetSymbolNameFromPathName(fileName, "dwg");
                //在打印进度框中显示的图纸名称：当前打印布局的名称（含文档名）
                plotDlg.set_PlotMsgString(PlotMessageIndex.SheetName, $"正在处理图纸{layout.LayoutName}({plotFileName}.dwg)");
            }
            PreviewEndPlotStatus status = PreviewEndPlotStatus.Cancel;//设置打印状态为停止
            PlotInfo pi = new PlotInfo();//创建打印信息
            pi.Layout = layout.ObjectId;//要打印的布局
            pi.OverrideSettings = ps;//使用ps中的打印设置
            //验证打印信息是否有效
            ...
            piv.MediaMatchingPolicy = MatchingPolicy.MatchEnabled;
            piv.Validate(pi);
            if (showProgressDialog) plotDlg.StartPlotProgress(false);
            //启动打印进度框
            engine.BeginPlot(plotDlg, null);//开启打印引擎进行打印任务
            //开始打印文档
            engine.BeginDocument(pi, layout.Database.GetDocument().Name, null, 1, plotToFile, fileName);
            if (plotDlg != null) StartSheetProgress(plotDlg);//启动打印图纸进程
            //开始打印页面
            PlotPageInfo ppi = new PlotPageInfo();
            engine.BeginPage(ppi, pi, true, null);
            engine.BeginGenerateGraphics(null);//开始打印内容
            if (plotDlg != null) plotDlg.SheetProgressPos = 50;//设置打印进度
            engine.EndGenerateGraphics(null);//内容打印结束
            ...
            engine.EndPage(pepi);
            status = pepi.Status;//打印预览结束时的状态
            if (plotDlg != null) EndSheetProgress(plotDlg);//终止显示打印图纸进程
            engine.EndDocument(null);//文档打印结束
            if (plotDlg != null) EndPlotProgress(plotDlg);//终止显示打印进度框
            engine.EndPlot(null);//结束打印任务
        ...
        public static void StartPlotProgress(this PlotProgressDialog plotDlg, bool isPreview)
        {
            ...
            plotDlg.set_PlotMsgString(PlotMessageIndex.SheetProgressCaption, "当前图纸进度：");
