...
namespace WinformUI.CADHelper
{
    ...
    public static class LayoutTools
    {
        ...
        /// <param name="bIncludeFirstViewport">是否将第一个视口包含在内或者将第二个视口包含在内</param>
        ...
        public static ObjectIdCollection GetEntsInLayout(this Layout layout, bool bIncludeFirstViewport)
        {
            ...
            foreach (ObjectId entId in btr)
            {
                ...
                var c = entId.GetObject(OpenMode.ForRead);
                if (vp != null)
                {
                    if (i == 0 && bIncludeFirstViewport)
                    {
                        entIds.Add(entId);
                    }
                    if (i == 1 && !bIncludeFirstViewport)
                    {
                        entIds.Add(entId);
                    }
                    i++;
            ...
            foreach (ObjectId btrId in bt)
            {
                ...
                if (btr.IsLayout)
                {
                    ...
                    if (layout.LayoutName.CompareTo(name) == 0)
                    {
                        ...
                        // 获取布局中的所有实体
                        entIds = layout.GetEntsInLayout(false);
                        break;
                    }
                }
            }
